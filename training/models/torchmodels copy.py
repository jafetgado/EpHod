"""
Modular functions and classes for building PyTorch models
"""
...
class LSTMModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_layers.append(nn.BatchNorm1d(lstm_dim))
        self.conv_layers.append(torchActivation(activation))
        self.conv_layers.append(nn.Dropout(conv_dropout))
    
        
        # Bidirectional LSTM layers
        self.lstm_layer = nn.GRU(lstm_dim, 
                                  lstm_dim, 
                                  batch_first=True, 
                                  bidirectional=False)
...
if __name__ == '__main__':
    ...
    model = LSTMModel(input_channel=20, 
                      input_length=1024, 
                      lstm_dim=32, 
                      conv_downsample=2, 
                      conv_dropout=0.1, 
    ...
    print(counted) # 23M to 69M parameters
    input_size = (3, 20, 1024)
    torchinfo.summary(model, input_size=input_size, 
                      col_names=['input_size', 'output_size', 'num_params'])
    x = torch.tensor(np.random.normal(size=(3,20,1024)), dtype=torch.float32)
    out = model(x)
    
    
    model = FeedForwardNetwork(input_dim=(20*1024), hidden_dims=[256,256], 
                               dropout=0.25, activation='relu', residual=True, 
                               random_seed=0)
    counted = count_parameters(model)
    print(counted) # 23M to 69M parameters
    input_size = (3, 20*1024)
